...
for(list in names(de.agreement.list)){
  print(list)
  print(de.agreement.list[[toString(list)]])
  
  de.agreement.list[[toString(list)]] %>% 
    as.data.frame() %>% 
    mutate(pathway = toString(list)) -> tmp.list
  
  pathway.list %>% 
    rbind(tmp.list) -> pathway.list
}
...
norm.counts.final %>% 
  ...
  t() %>% as.data.frame() %>% 
  pheatmap(show_rownames = F, show_colnames = F, annotation_colors = anno_colors, legend = T,
           ...
           annotation_col = luad.phenotypes.merge %>% select(sample, KRAS) %>% 
             ...
             filter(sample != 'TCGA-44-3917-01') %>% column_to_rownames('sample')) -> g12.v.healthy.for.reem.pdf

save_pheatmap_pdf(g12.v.healthy.for.reem.pdf, filename = file.path(output.data.dir, 'g12.v.healthy.for.reem.pdf.pdf'), 
                  width = panel.figure.width.in*1.5, height = panel.figure.height.in/3)
...
norm.counts.final %>% 
  ...
  mutate(KRAS = ifelse(grepl('p.G', KRAS), 'G12/3', KRAS)) %>% 
  group_by(KRAS) %>% 
  filter(n() >= length(de.agreement.list$up.consensus)*3) %>%
...
surv.fit <- survfit(as.formula(surv.obj ~ myc.avg.counts.tidy.stratified$rnaEasy), 
...
ggsurvplot(surv.fit, 
           ...
           pval.coord = c(0, 0.08),
           palette = viridis(20, option = 'magma')[c(6,18)]) 

panel.save(figure = 'supplement', name = 'luad.survival.for.reem', height = 'single', width = 'single')
...
toil.expected.counts.final %>% 
  ...
  mutate(across(where(is.numeric), round)) -> toil.expected.counts.matrix
...
norm.counts.final %>% 
  ...
  mutate(sample_type = as.factor(ifelse(grepl('WT', KRAS), 0, 1))) %>% select(-KRAS) %>%
  filter(sample != 'TCGA-44-3917-01') %>% 
  ...
  select(-Class) %>% 
  initial_split(prop = .75, strata = 'sample_type') -> toil.init.split
...
confusionMatrix(data = relevel(norm_pred, ref = "1"),
                reference = relevel(toil.test.split$sample_type, ref = '1'))
...
confusionMatrix(data = relevel(penalized_pred, ref = '1'),
                reference = relevel(toil.test.split$sample_type, ref = '1'))
#####
rf_pred <- predict(toi.rf.model, toil.test.split)

confusionMatrix(data = relevel(rf_pred, ref = "1"),
                reference = relevel(toil.test.split$sample_type, ref = '1'))
...
salmon.quant$deseq$counts %>% 
  ...
  mutate(sample_type = as.factor(ifelse(grepl('DMSO', sample), 1, 0))) %>% 
  column_to_rownames('sample') -> exRNA.test

norm_pred <- predict(norm_model, exRNA.test)

confusionMatrix(data = relevel(norm_pred, ref = "1"),
                reference = relevel(exRNA.test$sample_type, ref = '1'))
...
rf_pred <- predict(toi.rf.model, exRNA.test)
...
tot.biotype.dist <- function(tx.counts){
  return(tx.counts %>% 
    ...
    mutate(biotype = ifelse(biotype %in% c('lncRNA', 'Mt_rRNA', 'protein_coding', 'retained_intron',
                                           'Alu', 'L1', 'ERV1'),
                            biotype, 'other'),
           biotype = factor(biotype, levels = c('protein_coding', 'retained_intron', 'Mt_rRNA', 'lncRNA',
                            'Alu', 'L1', 'ERV1'))) %>% 
    group_by(biotype, sample, condition) %>% 
    summarise(count = sum(count)) %>% 
    group_by(sample) %>% 
...
panel.save('supplement', 'rnaEasy.3D.for.reem')
# gencode.35.tx.to.gene %>% 
...
salmon.quant$exoTIC_2D$deseq$de %>% 
  ...
  ggplot(aes(log2FoldChange.x, log2FoldChange.y, color = biotype)) + 
    geom_point() +
    xlab('exoTIC_2D') + ylab('luad_G12_v_WT') + 
    geom_hline(yintercept = 0, linetype = 'dashed', alpha=0.2) + 
...
salmon.quant$exoTIC_2D$deseq$H.fgsea %>%
  ...
  pheatmap(color = viridis(32, option = 'magma'), fontsize = 8, 
           treeheight_row = 2, treeheight_col = 2) -> h.fgsea.for.reem

save_pheatmap_pdf(h.fgsea.for.reem, filename = file.path(output.data.dir, 'h.fgsea.for.reem.pdf'), 
...
pca.out %>% 
  ...
  # mutate(KRAS = ifelse(grepl('p.G', KRAS), 'G12/13', ifelse(is.na(KRAS), 'n/a', 'otherMut'))) %>% 
