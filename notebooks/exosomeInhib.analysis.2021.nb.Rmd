...
toil.expected.counts <- read_csv(file.path(output.data.dir, 'tcga.data', 'TcgaTargetGtex_gene_expected_count.gz')) %>%
  select('ensg' = gene, contains('TCGA')) %>% 
  discard(~sum(.x == 0)/length(.x) >= 0.8) %>% 
  gather(sample, count, -ensg)
...
norm.counts.final %>% 
  ...
  filter(gene %in% c(de.agreement.list$dn.twoD)) %>%
  ...
  t() %>% as.data.frame() %>% 
  pheatmap(show_rownames = T, show_colnames = F, annotation_colors = anno_colors,
...
surv.fit <- survfit(as.formula(surv.obj ~ myc.avg.counts.tidy.stratified$up.consensus), 
                    data = myc.avg.counts.tidy.stratified)

ggsurvplot(surv.fit, 
           pval = T, 
           ggtheme = theme_set_fun() + theme(legend.position = c(0.95,0.95)), 
           palette = viridis(20, option = 'magma')[c(6,18)]) 

# panel.save(figure = 4, name = 'tcag.paad.myc.gene.set.km', height = 'single', width = 'single')
