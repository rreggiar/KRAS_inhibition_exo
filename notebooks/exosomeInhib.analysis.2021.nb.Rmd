...
norm.counts.final %>% 
  ...
  pheatmap(show_rownames = F, show_colnames = F, annotation_colors = anno_colors, legend = T,
           ...
           annotation_col = luad.phenotypes.merge %>% select(sample, KRAS) %>% 
             ...
             filter(sample != 'TCGA-44-3917-01') %>% column_to_rownames('sample'))
...
norm.counts.final %>% 
  ...
  geom_boxplot() + 
  geom_sina() +
  stat_pvalue_manual(perm.p.df, y.position = 13.5)
...
observed <- broom::glance(t.test(score ~ factor(KRAS), data = tmp.df, alternative = 'greater'))$estimate
