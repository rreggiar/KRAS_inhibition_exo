...
gencode.de.volcano <- function(de.seq){
  ...
  ggplot(de.seq, 
      ...
      geom_text_repel(aes(label = ifelse(padj <= label.max & log2FoldChange <= -x.max/1.5| log2FoldChange >= x.max/1.5,
                              ...
                              segment.alpha = 0.2,
                              color = 'black',
                              ylim = c(0, Inf), size = base_size/2, family = 'Helvetica')
}
...
gencode.de.volcano(salmon.quant$rnaEasy_2D$deseq$de) + theme(legend.position = c(0.95, 0.95))
gencode.de.volcano(salmon.quant$rnaEasy_3D$deseq$de) + theme(legend.position = c(0.95, 0.95))

gencode.35.tx.to.gene %>% 
  filter(gene %in% filter(salmon.quant$rnaEasy_3D$deseq$de, grepl('^LINC', gene))$gene) %>% 
  merge(as.data.frame(rowRanges(salmon.quant$txi)), by.x = 'enst', by.y = 'tx_name') %>% 
  distinct() %>% 
...
gene.patient.normalized.counts.pca <-  
  ...
  # dds.gxi.df %>%
  # dds.gxi.norm.counts %>% 
   ucsc.rmsk.salmon.quant$rnaEasy_all$deseq$counts %>%
