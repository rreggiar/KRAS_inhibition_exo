...
build.de.agreement.lists <- function(exoTIC_2D, rnaEasy_2D, rnaEasy_3D){
  ...
  exoTIC_2D %>% 
    ...
    filter(log2FoldChange.x >= 1 & log2FoldChange.y >= 1 & log2FoldChange >= 1) %>% 
    select(gene) %>% 
  ...
  exoTIC_2D %>% 
    merge(rnaEasy_2D, by = 'gene') %>% 
    filter(log2FoldChange.x >= 1 & log2FoldChange.y >= 1) %>% 
    select(gene) %>% 
...
anno_colors <- list(
  ...
  KRAS = c(G12C = 'blue', otherMut = 'orange'),
  TP53 = c(Mut = 'purple', WT = 'green')
)
...
norm.counts.final %>% 
  ...
  pheatmap(show_rownames = T, show_colnames = F, annotation_colors = anno_colors,
           annotation_col = luad.phenotypes.merge %>% 
             select(sample, sample_type, KRAS, TP53) %>% 
             mutate(KRAS = paste(KRAS), TP53 = paste(TP53)) %>% 
             mutate(KRAS = ifelse(grepl('G12C', KRAS), 'G12C', ifelse(is.na(KRAS), 'n/a', 'otherMut')),
                    TP53 = ifelse(grepl('NULL', TP53), 'WT', ifelse(is.na(TP53), 'n/a', 'Mut'))) %>% 
...
myc.avg.counts.tidy.stratified <-
  ...
  filter(gene %in% de.agreement.list$consensus) %>%
  select(sample, count) %>% 
  group_by(sample) %>%
...
surv.fit <- survfit(as.formula(surv.obj ~ myc.avg.counts.tidy.stratified$myc), data = myc.avg.counts.tidy.stratified)

ggsurvplot(surv.fit, 
           pval = T, 
           ggtheme = theme_set_fun(), 
...
norm.counts.final %>% 
  ...
  select(gene, sample, count, KRAS) %>%
  # mutate(sample_type = as.factor(ifelse(sample_type == 'Solid Tissue Normal', 0, 1))) %>% 
  mutate(sample_type = as.factor(ifelse(grepl('G12|G13', KRAS), 1, 0))) %>% select(-KRAS) %>% 
  filter(sample != 'TCGA-44-3917-01') %>% 
  mutate(count = log(count + 1)) %>% 
  pivot_wider(names_from = gene, values_from = count, values_fn = mean) %>% 
  column_to_rownames('sample') %>% 
  initial_split(prop = .60, strata = 'sample_type') -> toil.init.split
...
gene.patient.normalized.counts.pca <- 
  norm.counts.final %>% 
  filter(gene %in% c(signature.list$myc$consensus,
