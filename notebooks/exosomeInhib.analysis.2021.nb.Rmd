...
## saving panels
```{r}
panel.figure.width = 89
panel.figure.height = 247/3
...
total.tx.to.gene <- 
  rbind(gencode.35.tx.to.gene %>% select(enst, ensg, gene, len, biotype),
...
read_csv(file.path(output.data.dir, 'exoTIC_3D_nano.csv')) %>%  
  #this appears to have one extra, low value size row
  cbind(head(read_csv(file.path(output.data.dir, 'exoTIC_2D_nano.csv')), -1) %>% 
          select(-size)) %>% 
    cbind(read_csv(file.path(output.data.dir, 'rnaEasy_2D_nano.csv')) %>% 
            select(-size)) %>% 
    gather(sample, value, -size) %>% 
    mutate(condition = sample) %>% 
    separate(condition, sep = '_', into = c('context', 'platform', 'condition')) -> nanosight.df
...
read_csv(file.path(output.data.dir, 'exoTIC_3D_nano.csv')) %>% 
  gather(sample, value, -size) %>% 
  mutate(condition = sample) %>% 
  separate(condition, sep = '_', into = c(NA, NA, 'condition', NA)) %>% 
  ...
  xlab('size (nm)') +
  xlim(0, 350) #+
  # geom_vline(xintercept = 179.5) +
  # geom_segment(x = 30, xend = 150, y = 120, yend = 120)

read_csv(file.path(output.data.dir, 'exoTIC_2D_nano.csv')) %>% 
  gather(sample, value, -size) %>% 
  mutate(condition = sample) %>% 
  separate(condition, sep = '_', into = c(NA, NA, 'condition', NA)) %>% 
  group_by(size, condition) %>%
  ...
  xlab('size (nm)') +
  xlim(0, 350) + 
  geom_vline(xintercept = c(258.00, 219.50, 242.80, 247.70)) +
  geom_segment(x = 30, xend = 150, y = 160, yend = 160)


read_csv(file.path(output.data.dir, 'rnaEasy_2D_nano.csv')) %>% 
  gather(sample, value, -size) %>% 
  mutate(condition = sample) %>% 
  separate(condition, sep = '_', into = c(NA, NA, 'condition', NA)) %>% 
  group_by(size, condition) %>%
...
run.de.seq <- function(tximeta.object){
  ...
  colData(tximeta.object$gxi) %>% 
    as.data.frame() %>% 
    mutate(sample = names) #%>% 
    # merge(batch.annotation %>% select(sample, context, V3:V8), by = 'sample') -> colData.gxi
  
  print(colnames(colData.gxi))
  ...
  if (grepl('all', tximeta.name)){
    ...
    dds.gxi <- DESeqDataSetFromMatrix(countData = count.matrix.gxi, 
                                      colData = colData.gxi, 
                                      design = as.formula(~condition + condition:context))
  ...
  colData(tximeta.object$txi) %>% 
    as.data.frame() %>% 
    mutate(sample = names) %>% 
    merge(batch.annotation %>% select(sample, context, V3:V4), by = 'sample') -> colData.txi
  ...
  if (!grepl('exoTIC|rnaEasy|_D', tximeta.name)){
    print('alt approach')
...
return(list('de' = dds.gxi.lfc, 'counts' = dds.gxi.norm.counts, 'txi.counts' = dds.txi.norm.counts))
...
salmon.quant$exoTIC_2D$deseq$fgsea <- make.fgsea.ranks(salmon.quant$exoTIC_2D$deseq$de)
salmon.quant$rnaEasy_2D$deseq$fgsea <- make.fgsea.ranks(salmon.quant$rnaEasy_2D$deseq$de)

salmon.quant$exoTIC_3D$deseq$fgsea <- make.fgsea.ranks(salmon.quant$exoTIC_3D$deseq$de)
salmon.quant$rnaEasy_3D$deseq$fgsea <- make.fgsea.ranks(salmon.quant$rnaEasy_3D$deseq$de)
...
```{r}
toil.phenotypes <- 
  read_tsv(file.path(output.data.dir, 'tcga.data/TcgaTargetGTEX_phenotype.txt.gz')) %>% 
  select(sample, 
...
luad.kras.snps <- 
  luad.phenotypes %>% 
  rename(submitter_id.samples = 'sample') %>% 
  select(sample, contains('KRAS')) %>% 
  drop_na()

toil.quant <- list('toil.survival' = toil.survival,
                   'toil.survival' = luad.phenotypes,
                   'luad.kras.snps' = luad.kras.snps)

toil.norm.counts <- read_csv(file.path(output.data.dir, 'tcga.data/toil.counts.csv'))

toil.quant$norm.counts.tidy <- 
  toil.norm.counts %>% 
  select('ensg' = gene, contains('TCGA'), contains('GTEX')) %>% 
  discard(~sum(.x == 0)/length(.x) >= 0.8) %>% 
  gather(sample, count, -ensg) %>% 
  merge(toil.phenotypes %>% filter(tissue == 'Lung'), by = 'sample') %>% 
...
toil.quant$norm.counts.tidy.stratified <- 
  ...
  # dplyr::rename(gene) %>%
  # # merge(toil.norm.count.gene.summary.values, by = 'ensg') %>% 
  # #filter(normal.sd > 0) %>% 
  group_by(gene) %>%
...
```{r}
biotype.dist.plt <- function(tx.counts){
  tx.counts %>% 
    ...
    # ggplot(aes(biotype, freq, fill = condition, color = condition)) +
    #   geom_boxplot() +
    #   # geom_sina() +
    #   scale_fill_viridis_d( direction = -1) +
    #   scale_color_viridis_d(direction = -1) +
...
####
```{r}
biotype.dist.plt(salmon.quant$deseq$txi.counts)

biotype.dist.plt(ucsc.rmsk.salmon.quant$deseq$txi.counts)
...
gencode.de.volcano(salmon.quant$exoTIC_3D$deseq$de)
gencode.de.volcano(salmon.quant$rnaEasy_2D$deseq$de)
gencode.de.volcano(salmon.quant$rnaEasy_3D$deseq$de)
...
salmon.quant$exoTIC_2D$deseq$de %>% 
  merge(salmon.quant$exoTIC_3D$deseq$de, by = 'gene') %>% 
  mutate(biotype = ifelse(gene %in% gencode.35.lnc.gene.names$gene, 'lncRNA','coding')) %>% 
  filter(!grepl('^RPS|^MT', gene)) %>% 
  ggplot(aes(log2FoldChange.x, log2FoldChange.y, color = biotype)) + 
    geom_point() +
    ylab('exoTIC_3D') + xlab('exoTIC_2D') + 
    geom_hline(yintercept = 0, linetype = 'dashed', alpha=0.2) + 
    geom_vline(xintercept = 0, linetype = 'dashed', alpha=0.2) +
    scale_color_manual(values = c(viridis(1)[[1]], viridis(3)[[2]], viridis(5)[[5]])) +
    geom_text_repel(aes(label = ifelse(log2FoldChange.x >= 1| log2FoldChange.y > 1,
                                         as.character(gene), '')),
                              box.padding = unit('1', 'lines'),
                              segment.color = 'grey',
                              segment.alpha = 0.2,
                              color = 'black',
                              ylim = c(-Inf, Inf), size = rel(2))
...
####
```{r}
gene.expression.hclust.plt(salmon.quant$exoTIC_3D$deseq$counts)





salmon.quant$exoTIC_3D$deseq$counts %>% 
...
te.de.volcano(ucsc.rmsk.salmon.quant$exoTIC_2D$deseq$de %>% mutate(log2FoldChange = -log2FoldChange))
te.de.volcano(ucsc.rmsk.salmon.quant$exoTIC_3D$deseq$de %>% mutate(log2FoldChange = -log2FoldChange))
te.de.volcano(ucsc.rmsk.salmon.quant$rnaEasy_2D$deseq$de %>% mutate(log2FoldChange = -log2FoldChange))
...
gene.patient.normalized.counts.pca <-  
  ...
  # dds.gxi.df %>%
  # dds.gxi.norm.counts %>% 
  ucsc.rmsk.salmon.quant$deseq$counts %>%
  ...
  mutate(gene.cv = rowSds(as.matrix(select(., contains('.'))))) %>%
...
pca.out %>% 
  ...
  ggplot(aes(PC1, PC2,
             color = platform,
             shape = condition, size = context)) + 
    geom_point(size = rel(2), alpha = 0.6) + 
...
# current.meta.data %>% 
#   rbind(covid.patient.metadata %>% mutate(diabetes = 0, batch = 5, condition = 'covid') %>% 
#           select(patient, gender, age, diabetes, condition, batch)) -> current.meta.data
...
pca_umap <- as.data.frame(umap(pca.out, n_neighbors = 7, learning_rate = 0.5, init = "random"))
